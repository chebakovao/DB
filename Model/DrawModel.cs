...
namespace DBChebakov.Model
{
    public class DrawModel
    {
        ...
        public void DrawFlag(double x, double y, double radius, Brush brush)
        {
            ...
            var stickEndPoint = new Point(x, y); 
            var stickGeometry = new LineGeometry(stickStartPoint, stickEndPoint);
            var stickDrawing = new GeometryDrawing(Brushes.Red, new Pen(brush, 0.1), stickGeometry);
            ...
            var triangleDrawing = new GeometryDrawing(brush, new Pen(brush, 0.5), triangleGeometry);
        ...
        public DrawingImage Render(int offset = 3, int grid = 10, bool drawAxes = false)
        {
            ...
            if ((minY < 0 && maxY > 0) || drawAxes) DrawLine(minX, 0, maxX, 0, "#ECECF0", 0.01);
            if ((minX < 0 && maxX > 0) || drawAxes) DrawLine(0, minY, 0, maxY, "#ECECF0", 0.01);

            for (double i = minX > 0 ? minX + grid - minX % grid : minX - minX % grid; i < maxX; i += grid)
            {
                DrawLine(i, minY, i, maxY, "Black", 0.08);
                for (double j = minY > 0 ? minY + grid - minY % grid : minY - minY % grid; j < maxY; j += grid)
                {
                    DrawLine(minX, j, maxX, j, "Black", 0.08);
                    DrawText($"{i}; {j}", i-0.7, j, Brushes.Red, 2);
                    DrawCircle(i, j, 0.1, "Red");
                }
            }            
