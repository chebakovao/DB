...
namespace DBChebakov.ViewModel
{
    public class MainViewModel : PropChange
    {
        ...
        {
            if (MessageBox.Show("Удалить заказчика?", "Удаление", MessageBoxButton.YesNo) == MessageBoxResult.No) return;
            else
            {
        ...
        {
            if (MessageBox.Show("Удалить проект?", "Удаление", MessageBoxButton.YesNo) == MessageBoxResult.No) return;
            else
            {
        ...
        {
            var project = new Project() { Name = "", Customer = SelectedCustomer };
        ...
        {
            if (MessageBox.Show("Удалить площадь?", "Удаление", MessageBoxButton.YesNo) == MessageBoxResult.No) return;
        ...
        void Redraw()
        {
            ...
            foreach (var area in SelectedProject?.Areas ?? new())
            {
                ...
                else if (area != SelectedArea && area.IsCorrect()) area.Draw(newimage, Brushes.Blue);
                else if (area != SelectedArea && !area.IsCorrect()) area.Draw(newimage, Brushes.Green);
                foreach (var profile in area.Profiles ?? new())
                {
                    if (area == SelectedArea && profile.IsCorrect()) profile.Draw(newimage, Brushes.Aqua);
                    else if (area == SelectedArea && !profile.IsCorrect()) profile.Draw(newimage, Brushes.DarkGray);
                    else if (area != SelectedArea && profile.IsCorrect()) profile.Draw(newimage, Brushes.Purple);
                    else if (area != SelectedArea && !profile.IsCorrect()) profile.Draw(newimage, Brushes.DarkRed);
                }
            }
